...
public class Bankomat {
    ...
    private List<Banknote> getAmountOut(long cash) {
        ...
        for (Map.Entry<Nominal, Deque<Banknote>> entry : this.banknotes.entrySet()) {
            ...
            if (!value.isEmpty() && cash >= nominalKey.getNominal()) {
                List<Banknote> bt = getBanknote(value, calcCountOfBanknotes(nominalKey, cash));
                banknotesOut.addAll(bt);
                cash -= (long) bt.size() * nominalKey.getNominal();
            }
        }
        return banknotesOut;
    }
